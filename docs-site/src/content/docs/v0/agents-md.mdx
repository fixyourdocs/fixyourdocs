...
- **Mode A — the first-party block.** Paste it into *your* repo's
  `AGENTS.md` so agents working there report *that repo's* docs. This is the
  one below.
...
The difference from Mode A is scope and consent: this block is **global**
(it travels with you, not a repo) and it tells the agent to **offer** —
public docs only, with your OK — rather than report silently.
