...
function authEvent(owner: string, repo: string, sub = 'user-1'): any {
  return {
    requestContext: {
      requestId: 't',
      http: { method: 'DELETE', path: '/v1/orgs/me/repos/x/y', sourceIp: '1.2.3.4' },
...
describe('DELETE /v1/orgs/me/repos/{owner}/{repo}', () => {
  it('owner deletes: 200 + lower-cased key, owner-scoped condition', async () => {
  ...
  it('non-owner / already gone → 404 (condition fails, no existence leak)', async () => {
  ...
  it('400 when owner/repo is missing', async () => {
