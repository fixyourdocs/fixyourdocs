...
// Dispatch ddb.send by the key it asks for: repo claims by `repo`, domain/pages
// rows by `domain`, the integration by `userId`. `rows` is one map keyed by the
// string value of whichever PK is asked for.
function route(rows: Record<string, unknown>, integrations: Record<string, unknown> = {}) {
  sendMock.mockImplementation((cmd: any) => {
    const key = cmd?.input?.Key ?? {};
    if (typeof key.repo === 'string') return Promise.resolve({ Item: rows[key.repo] });
...
describe('resolveTargetForReport — repo-file URLs', () => {
  ...
  it('routes a blob/HEAD URL to its repo claim', async () => {
    route({ [repoKey('acme', 'widgets')]: repoClaim('Acme', 'Widgets') }, { u1: integration() });
  ...
  it('returns null when the claim exists but its owner lost the installation', async () => {
    route({ [repoKey('acme', 'widgets')]: repoClaim('Acme', 'Widgets') }, { u1: { userId: 'u1', status: 'installed' } });
...
describe('resolveTargetForReport — GitHub Pages URLs (auto-derived)', () => {
  ...
  it('derives the user/org-Pages repo (<user>/<user>.github.io)', async () => {
    route({ [repoKey('acme', 'acme.github.io')]: repoClaim('acme', 'acme.github.io') }, { u1: integration() });
    ...
    route(
      {
        ...
        [repoKey('acme', 'docs')]: repoClaim('Acme', 'Docs'),
      },
      { u1: integration() },
    );
